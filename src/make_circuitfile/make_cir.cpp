...
int make_cir(double new_value, int ele_num, vector<ele_unit> &element, vector<string> &data_cir){
    ...
    for (int x = 0; x < element.size(); x++) {
        ...
        switch(element[x].ide_num){ // マージンを測定している素子なら新しいデータに書き換える
                case 0:
                    liness <<  element[x].element << fixed  << right << setw(6) << element[x].node1 << setw(6) << element[x].node2 << setw(20) <<  setprecision(3) << value << element[x].unit << "  fcheck" ;
                ...
                case 3:   //BI
                    ...
                    break; 
                case 4:   //F
                    liness <<  element[x].element << fixed  << right << setw(6) << element[x].node1 << setw(6) << element[x].node2 << setw(20) <<  setprecision(3) << value << element[x].unit;
                    data_cir_copy[y] = liness.str();
                    break;
                case 5:   //R
                    liness <<  element[x].element << fixed  << right << setw(6) << element[x].node1 << setw(6) << element[x].node2 << setw(20) <<  setprecision(3) << value << element[x].unit;
                ...
                case 6:   //V
                    liness <<  element[x].element << fixed  << right << setw(6) << element[x].node1 << setw(6) << element[x].node2 << "   "
                        << "PWL(0ps 0mV " <<setw(10) <<  setprecision(3) << "50ps   " << setw(20) <<  setprecision(3)<< value << element[x].unit <<  ")";
                    data_cir_copy[y] = liness.str();
                    break;
                case 7:   //I
                    liness <<  element[x].element << fixed  << right << setw(6) << element[x].node1 << setw(6) << element[x].node2 << "   "
