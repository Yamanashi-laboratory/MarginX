...
void Margin_syn(vector<ele_unit> &element, vector<vector<judge>> &jud, vector<string> &data_cir, vector<string> &arg_arr, int menu){
    if(menu == 0 || menu == 2){
            ...
            board  *shmaddr;
                //マルチプロセスL
                    ...
                    if(pid[i] == 0){
                        ...
                        margin_ele_syn(i,element,  jud, shmaddr, data_cir);
                        if (shmdt(shmaddr) == -1) {
                            exit(EXIT_FAILURE);
                        }
                        exit(0);
