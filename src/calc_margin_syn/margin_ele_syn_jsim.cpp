...
/* 全素子のマージン測定 */  
int margin_ele_syn_jsim(int ele_num, vector<ele_unit> &element, vector<vector<judge>> &jud, board *top, vector<string> &data_cir){
    int check = 0;
 ...
 /*素子の値が正の場合*/
    if(element[ele_num].value > 0){                      
        ...
        while(check < 4){
            ...
            else if(judge_operation_jsim(jud, 0) == 0){
                HIGH = HIGH - pow(10 , order_num - check);
                synchro(copy, ele_num, HIGH);
                check += 1;
            }
