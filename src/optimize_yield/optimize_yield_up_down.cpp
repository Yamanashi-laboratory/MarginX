...
void optimize_yield_up_down(vector<ele_unit> &element, vector<string> &data_cir, vector<vector<judge>> &jud, vector<string> &arg_arr){
    ...
    for (int m = 0; m < MONTE_CARLO; m++){
        ...
        if(reduce(begin(yield_his), end(yield_his)) / yield_his.size() > suc_th && pm_key == 0){  //直近 5 サイクルの歩留まり(success) の平均が 60 を超えたら、標準偏差を +0.01
            p_key = 1;
            local_nd += 0.01;
            Margin(element,  jud, data_cir, arg_arr, 2);
            not_upd = 0;
        ...
        if(not_upd > 100 && pm_key == 0){  //直近 5 サイクルの歩留まり(success) の平均が 60 を超えない状況が50 回続いたら、標準偏差を -0.01
            m_key = 1;
            local_nd -= 0.01;
            Margin(element,  jud, data_cir, arg_arr, 2);
            not_upd = 0;
