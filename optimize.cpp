...
void optimize(vector<ele_unit> &element, vector<string> &data_cir, ele_cou *cou,vector<string> &elej, vector<vector<judge>> &jud, vector<string> &arg_arr){
    ...
    for (int m = 0; m < MONTE_CARLO; m++){
        ...
        double global = rand_global(gau, element);
        vector<int> pid;
        //マルチプロセス開始
        for (int i = 0; i < MULTI_NUM; ++i) { //MULTI_NUM = 1回のモンテカルロで生成するマルチプロセス数
                ...
                if(pid[i] == 0){       
                    ...
                    opt_ele(element,data_cir,cou,elej,jud,shmaddr,m ,i, global);
...
void opt_ele(vector<ele_unit> &element, vector<string> &data_cir, ele_cou *cou,vector<string> &elej, vector<vector<judge>> &jud, opt_num *opt, int m, int i, double global){
...
double rand_global(gauss *gau, vector<ele_unit> &element){
    std::random_device rnd;
    std::mt19937 mt(rnd()); 
...
void optimize_yield(vector<ele_unit> &element, vector<string> &data_cir, ele_cou *cou,vector<string> &elej, vector<vector<judge>> &jud, vector<string> &arg_arr){
